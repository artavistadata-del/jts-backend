...
# def process_finance_excel(raw_bytes: io.BytesIO) -> tuple[pl.DataFrame, pl.DataFrame]:
#     """
#     Menerima file mentah (memory), memproses sheet BS dan IS, 
#     dan mengembalikan 2 dataframe Polars yang sudah bersih.
#     """
    
#     # ==========================================
#     # 1. PROSES SHEET "BS"
#     # ==========================================
#     # Baca data dan jadikan baris pertama sebagai header
...
#         pl.col(['Idx Category', 'Idx Sub Category']).cast(pl.Int8),
...
#         pl.col('Idx Sub Category').cast(pl.Int8),
#         pl.col('Value').cast(pl.Float64, strict=False),
#         pl.col('Bulan').str.to_datetime().cast(pl.Date)
#     )

#     # 3. Kembalikan 2 output (Tuple berisi 2 DataFrame)
#     return df_bs_clean, df_is_clean
...
def process_finance_excel(raw_bytes: io.BytesIO, history_id: int) -> list[dict]:
    """
    Menerima file mentah (memory), memproses sheet BS dan IS, 
    menggabungkannya, dan mengembalikan List of Dictionary 
    yang siap di-Bulk Insert ke PostgreSQL.
    """
    ...
    # ==========================================
    # Reset pointer BytesIO agar excel bisa dibaca dari awal lagi untuk sheet kedua
    raw_bytes.seek(0) 
