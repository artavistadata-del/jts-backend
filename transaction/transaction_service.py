from models.models.models import RoleEnum
...
class TransactionService:
    ...
    def list_transactions(self, id_dept: int, page: int, size: int, user_role: str, user_id: int):
        # 1. Ambil model dinamis sesuai departemen (misal: FactFinance)
        config = get_dept_config(id_dept)
        target_model = config["model"]
        
        skip = (page - 1) * size
        
        # 2. Penentuan Filter: Jika Manager, nik_filter = None (Ambil Semua)
        # Jika bukan Manager (Staff), nik_filter = NIK Staff tersebut
