...
def _clean_sheet3(raw_bytes: BytesIO, history_id: int) -> pl.DataFrame:
    ...
    df = df.with_columns(
        pl.when(pl.col("detail").str.contains("$/ton", literal=True))
        .then(pl.lit("($/ton)"))

        .when(pl.col("detail").str.contains("$/M", literal=True))
        .then(pl.lit("($/M)"))

        .when(pl.col("detail").str.contains("ton/M", literal=True))
        .then(pl.lit("(ton/M)"))

        .when(pl.col("detail").str.contains("($)", literal=True))
        .then(pl.lit("($)"))
