...
class Departments(Base):
    ...
    users: Mapped[list['Users']] = relationship('Users', back_populates='departments')
    history: Mapped[list['History']] = relationship('History', back_populates='departments_history')
...
class Roles(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[Optional[RoleEnum]] = mapped_column(
        Enum(RoleEnum, values_callable=lambda cls: [member.value for member in cls], name='role_enum', schema='oltp_main')
    )
    public_id = Column(String(22), unique=True, index=True, default=shortuuid.uuid)

    users: Mapped[list['Users']] = relationship('Users', back_populates='roles')
    history: Mapped[list['History']] = relationship('History', back_populates='roles_history')
...
class Users(Base):
    ...
    roles_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.roles.id'), index=True)
    departments_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.departments.id'), index=True)
    ...
    departments: Mapped['Departments'] = relationship('Departments', back_populates='users')
    roles: Mapped['Roles'] = relationship('Roles', back_populates='users')
    history: Mapped[list['History']] = relationship('History', back_populates='users')
...
class History(Base):
    ...
    users_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.users.id'), index=True)
    roles_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.roles.id'), index=True)
    departments_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.departments.id'), index=True)
    ...
    users: Mapped['Users'] = relationship('Users', back_populates='history')
    roles_history: Mapped['Roles'] = relationship('Roles', back_populates='history')
    departments_history: Mapped['Departments'] = relationship('Departments', back_populates='history')
    finance_data: Mapped[list['FactFinance']] = relationship('FactFinance', back_populates='history', cascade="all, delete-orphan")
...
class FactFinance(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    history_id: Mapped[int] = mapped_column(Integer, ForeignKey('oltp_main.history.id', ondelete="CASCADE"), index=True)
    ...
    history: Mapped['History'] = relationship('History', back_populates='finance_data')