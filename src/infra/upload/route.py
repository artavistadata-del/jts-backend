...
def get_upload_status(
    ...
    if record.status == StatusEnum.FAILED:
        error_msg = (
            record.analysis_result.get("error")
...
def confirm_upload(
    ...
    return {"status": "success", "message": "Berhasil Melakukan Aksi"}
...
def get_staging_finance_transactions_endpoint(
    ...
    try:
        result = service.get_staging_finance_transactions(
            history_id=history_id, skip=skip, limit=limit, report_type=report_type
        )
        return result
